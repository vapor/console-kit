...
extension CommandInput {
    public func argument(_ name: String) throws -> String {
        guard let arg = arguments[name] else {
            throw ConsoleError(identifier: "missingArgument", reason: "No argument named `\(name)` exists in the command signature.")
        }
    ...
    public func assertOption(_ name: String) throws -> String {
        guard let option = options[name] else {
            throw ConsoleError(identifier: "missingOption", reason: "Option `\(name)` is required and was not supplied.")
        }
